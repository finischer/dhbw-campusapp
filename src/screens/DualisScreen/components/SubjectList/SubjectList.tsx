...
import { ISubjectListProps, ISubjectListRenderItemProps } from "./subjectList.types";
...
const SubjectList: React.FC<ISubjectListProps> = ({ subjects, requestTime, onRefresh = undefined }) => {
  ...
  const showSubjectList = filteredSubjects.length === 0 ? subjects : filteredSubjects;

  // List all semesters (unique semesters)
  const allSemesters = Array.from(new Set(subjects.map((subject) => subject.semester)));
  ...
  const filterElements = allSemesters.map((semester: string) => {
    ...
    return (
      <View
        key={semester}
        style={subjectListStyles.chipContainer}
      >
        <Chip
          label={semester}
  ...
  const toggleFilter = (semester: string) => {
    if (selectedSemesters.includes(semester)) {
      const newSelectedSemesters = selectedSemesters.filter((s) => s !== semester);
      setSelectedSemesters(newSelectedSemesters);
      return;
    }

    setSelectedSemesters((oldSelectedSemesters) => [...oldSelectedSemesters, semester]);
  };
  ...
  return (
    <Animated.View
      ...
      style={{ flex: 1 }}
    >
      <RegularText style={subjectListStyles.semesterTitleText}>{t("semester")}</RegularText>
      <View style={subjectListStyles.filterContainer}>{filterElements}</View>
      <RegularText style={subjectListStyles.subjectListTitleText}>{t("subjects")}</RegularText>
      <FlatList
        ...
        ListFooterComponent={<RequestTime requestTime={requestTime} />}
        refreshControl={
          <RefreshControl
